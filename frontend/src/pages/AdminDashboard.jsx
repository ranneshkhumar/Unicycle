...
export default function AdminDashboard() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const [d, u, i] = await Promise.all([API.get('/admin/dashboard'), API.get('/admin/users'), API.get('/admin/items')]);
  ...
  const handleRemoveItem = async (id) => {
    ...
    try { await API.delete(`/admin/items/${id}`); toast.success('Removed'); fetchAll(); }
    catch { toast.error('Failed'); }
  };

  const handleToggleUser = async (id) => {
    try { await API.put(`/admin/users/${id}/toggle`); toast.success('Updated'); fetchAll(); }
    catch { toast.error('Failed'); }
  };
...
const s = {
  ...
  tab: { padding: '9px 20px', borderRadius: 8, border: '1.5px solid #E2EFE6', background: '#fff', color: '#6B7280', fontWeight: 500, cursor: 'pointer', fontSize: 14, transition: 'all 0.2s', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' },
  ...
  summaryGrid: { display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12 },
  ...
  th: { padding: '10px 14px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#6B7280', textTransform: 'uppercase', letterSpacing: 0.8, borderBottom: '1.5px solid #E2EFE6' },
  ...
  actionBtn: { padding: '5px 12px', borderRadius: 6, fontWeight: 600, cursor: 'pointer', fontSize: 12 },
