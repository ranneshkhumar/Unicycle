...
export default function Messages() {
  ...
  const fetchInbox = async () => {
    try { const { data } = await API.get('/messages/inbox'); setConversations(data.conversations); }
    catch {}
  };
  ...
  const fetchConversation = async (uid) => {
    try {
      ...
      if (data.messages.length > 0) {
        ...
        try { const r = await API.get(`/users/${uid}`); setOtherUser(r.data.user); } catch {}
      }
    } catch {}
  };
              ...
              conversations.map(conv => {
                ...
                return (
                  <div key={conv._id}
                    ...
                    onClick={() => { setActiveConv(other?._id); setOtherUser(other); }}>
            ...
            {!activeConv ? (
              ...
              <>
                      ...
                      return (
                        <div key={msg._id} style={{ ...s.msgRow, justifyContent: isMine ? 'flex-end' : 'flex-start' }}>
                          ...
                          <div style={{ ...s.bubble, ...(isMine ? s.myBubble : s.theirBubble) }}>
                            ...
                            <p style={s.msgTime}>{new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                          </div>
                        </div>
                      );
                    })
                ...
                <form onSubmit={handleSend} style={s.inputRow}>
                  <input style={s.inputField} type="text" placeholder="Type a message..." value={text} onChange={e => setText(e.target.value)} />
                  <button style={{ ...s.sendBtn, ...(!text.trim() || loading ? s.sendDisabled : {}) }} type="submit" disabled={loading || !text.trim()}>Send ➤</button>
                </form>
              </>
            )}
          </div>
...
const s = {
  ...
  layout: { display: 'grid', gridTemplateColumns: '290px 1fr', gap: 20, height: '72vh' },
  inbox: { background: '#fff', border: '1px solid #E2EFE6', borderRadius: 14, padding: 16, overflowY: 'auto', boxShadow: '0 2px 8px rgba(46,125,50,0.06)' },
  ...
  convItem: { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 8px', borderRadius: 10, cursor: 'pointer', transition: 'all 0.2s', marginBottom: 4 },
  ...
  chatBox: { background: '#fff', border: '1px solid #E2EFE6', borderRadius: 14, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 2px 8px rgba(46,125,50,0.06)' },
  ...
  bubble: { maxWidth: '68%', padding: '10px 14px', borderRadius: 16, wordBreak: 'break-word' },
  ...
  inputField: { flex: 1, padding: '11px 16px', background: '#F9FBF9', border: '1.5px solid #E2EFE6', borderRadius: 50, color: '#1B1B1B', fontSize: 14 },
  sendBtn: { padding: '11px 20px', background: '#2E7D32', color: '#fff', border: 'none', borderRadius: 50, fontWeight: 600, cursor: 'pointer', fontSize: 14, boxShadow: '0 2px 8px rgba(46,125,50,0.2)' },
